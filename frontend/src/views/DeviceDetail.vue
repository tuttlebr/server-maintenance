...
import { deleteDevice, getDevice, scanDevice, updateDevice, updateDeviceAnnotations } from "../api.js";
...
const router = useRouter(); const device = ref(null); const loading = ref(true); const scanning = ref(false); const showEdit = ref(false); const showRemove = ref(false); const activeTab = ref("Summary"); const tabs = ["Summary", "Hardware", "Software", "Context", "Operations"]; const savingAnnotations = ref(false); const annotationRows = ref([]); let nextAnnotationId = 1;
...
async function load() { loading.value = true; try { device.value = await getDevice(props.id); Object.assign(edit, { display_name: device.value.name, endpoint: device.value.endpoint, ssh_user: "" }); resetAnnotationRows(); } catch (error) { device.value = null; } finally { loading.value = false; } }
...
@media (max-width: 760px) { .detail-actions { margin-top: 14px; justify-content: flex-start; flex-wrap: wrap; }.detail-grid, .context-grid { grid-template-columns: 1fr; }.detail-grid .full { grid-column: auto; }.detail-title-row { flex-wrap: wrap; }.annotation-row { grid-template-columns: 1fr auto; }.annotation-row input:nth-child(2) { grid-column: 1; } }
</style>
